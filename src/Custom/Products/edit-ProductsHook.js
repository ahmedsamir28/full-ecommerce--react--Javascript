...
import { getAllBrand, getAllBrands } from '../../redux/actions/brandsActions';
...
const AdminEditProductsHook = (id) => {
    ...
    const [priceAftr, setPriceAftr] = useState('The price after discount');
    ...
    const handelSubmit = async (e) => {
        ...
        if (CatID === 0 || prodName === "" || prodDescription === "" || images.length <= 0 || priceBefore <= 0) {
            notify("Please complete the information", "warn")
            return;
        }
    ...
    useEffect(() => {
        ...
        if (loading === false) {
            ...
            setPriceAftr('The price after discount')
